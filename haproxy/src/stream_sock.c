...
static int stream_sock_write_loop(struct stream_interface *si, struct buffer *b)
{
	...
	struct session *s = (struct session *)((struct task *)si->owner)->context;
	...
	/*
	 *  here we record the l7 delay don's include connect to backend server time.
	 *  in the main loop, time will upate after handle all running task, before fired file events
	 *  and if the request is invalid, we don't need to calc the delay time.
	 */
	if ((si->connect != NULL) && 
		tv_iszero(&s->logs.tv_forward_req) && 
		ret > 0)
	{
		s->logs.tv_forward_req = now;
	}
