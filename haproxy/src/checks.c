...
static void set_backend_down(struct proxy *be)
{
	...
	send_log(be, LOG_WARNING, "%s %s has no server available!\n", proxy_type_str(be), be->id);
}
...
static int event_srv_chk_r(int fd)
{
	...
	if (s->proxy->options & PR_O_HTTP_CHK) {
		...
		/* check the reply : HTTP/1.X 2xx and 3xx, 4xx are OK */
		else if (*(s->check_data + 9) == '2' || *(s->check_data + 9) == '3' || *(s->check_data + 9) == '4') {
			cut_crlf(desc);
 ...
 */
int start_check(struct proxy *px)
{
	struct server *s;
	struct task *t;
