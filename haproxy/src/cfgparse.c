...
void clear_acl_rule (struct proxy *px);
...
/* proxy->options3 */
static const struct cfg_opt cfg_opts3[] =
{
	{ "server_from_file",             PR_O3_SERVER_FROM_FILE, PR_CAP_BE, 0},
	{ "hash-filename",                PR_O3_HASH_FILENAME,   PR_CAP_BE, 0, PR_MODE_HTTP },
	{ "change2xforwardfor",           PR_O3_Change2Forward,   PR_CAP_FE, 0, PR_MODE_HTTP},
	{ "acl_from_file",           	 PR_O3_ACL_FROM_FILE,   PR_CAP_FE, 0, PR_MODE_HTTP},
	{ NULL, 0, 0, 0 }
};
...
static int create_cond_regex_rule(const char *file, int line,
		...
		const char **cond_start, int reload);
/**
 * parse the acl line of the acl_file(7layer rule)
 * Return the error code , 0 if OK, other -1
 * args: acl name conds
 */

int process_acl_line (char** args, struct proxy *px, char* file, int linenum, int reload)
{
	const char* err;

	if (NULL == args || NULL == px) 
		return ERR_ALERT;
	...
	if (reload) {
		if (parse_acl((const char **)args + 1, &px->acl_new) == NULL) {
			Alert("parsing [%s:%d] : error detected while parsing ACL '%s'.\n",
				file, linenum, args[1]);
			return ERR_ALERT;
		}
	} else {
		if (parse_acl((const char **)args + 1, &px->acl) == NULL) {
			Alert("parsing [%s:%d] : error detected while parsing ACL '%s'.\n",
				file, linenum, args[1]);
			return ERR_ALERT;
		}
...
int warnif_misplaced_block(struct proxy *proxy, const char *file, int line, const char *arg, int reload);
int warnif_rule_after_use_backend(struct proxy *proxy, const char *file, int line, const char *arg, int reload);

/**
 * process "block" directive. 
 * reload false: first parse the file, true: reload the acl_file
 */ 
int process_block_line(char** args, struct proxy *curproxy, char* file, int linenum, int reload) 
{
	int err_code = 0;
	...
	if ((cond = build_acl_cond(file, linenum, curproxy, (const char **)args + 1, reload)) == NULL) {
	...
	if (reload == ACL_RELOAD) {
		LIST_ADDQ(&curproxy->block_cond_new, &cond->list);
	} else {
		LIST_ADDQ(&curproxy->block_cond, &cond->list);
	}
	warnif_misplaced_block(curproxy, file, linenum, args[0], reload);
out:
	return err_code;
}

int process_redirect_line (char** args, struct proxy *curproxy, 
		char* file, int linenum, int reload)
{
	...
	while (*(args[cur_arg])) {
		if (!strcmp(args[cur_arg], "location")) {
			...
			cond = build_acl_cond(file, linenum, curproxy, (const char **)args + cur_arg, reload);
			if (!cond) {
	...
	LIST_INIT(&rule->list);
	if (reload) {
		LIST_ADDQ(&curproxy->redirect_rules_new, &rule->list);
	} else {
		LIST_ADDQ(&curproxy->redirect_rules, &rule->list);
	}
	warnif_rule_after_use_backend(curproxy, file, linenum, args[0], reload);
...
/**
 * process "use_backend" directive. 
 * reload false: first parse the file, true: reload the acl_file
 */ 
int process_use_backend_line(char** args, struct proxy *curproxy, 
		char* file, int linenum, int reload)
{
	...
	if ((cond = build_acl_cond(file, linenum, curproxy, (const char **)args + 2, reload)) == NULL) {
	...
	// only waring
	warnif_cond_requires_resp(cond, file, linenum);


	rule = (struct switching_rule *)calloc(1, sizeof(*rule));
	if (NULL == rule) {
		Alert("Memory is not enough return. \n");
		return -1;
	...
	if (reload) {
		LIST_ADDQ(&curproxy->switching_rules_new, &rule->list);
	} else 
...
/**
 * validate the hot reloaded acl rule
 * success return 0
 */
int validate_newacl_rule (struct proxy *px)
{

	int need_rollback = 0;
	struct switching_rule *rule = NULL;
	//validate the switching rule
	list_for_each_entry(rule, &px->switching_rules_new, list) {

		struct proxy *target;
		target = findproxy_mode(rule->be.name, px->mode, PR_CAP_BE);
		...
		if (!target) {
		...
		} else if (target == px) { // compare to orig px
			...
			/*ok,let's check the default backend cofigure status*/
			if(target->conf.flags & PROXY_CONFIG_INVALID) {
				need_rollback = 1;
				break;
			}
	...
	if (need_rollback) {
		Alert ("Proxy: %s update new acl rules failed, will abort this proxy\n", px->id);
		return -1;
	}

	return 0;
}
void list_exchange(struct list *new, struct list *old)
{
	struct list *tmp;

	if (LIST_ISEMPTY(new) && LIST_ISEMPTY(old))
	{
		return;
	}
...
int replace_acl_rule(struct proxy *px)
{
	clear_acl_rule(px);
	// not empty
	list_exchange(&px->acl_new, &px->acl);
	list_exchange(&px->block_cond_new, &px->block_cond);
	list_exchange(&px->switching_rules_new, &px->switching_rules);
	list_exchange (&px->redirect_rules_new, &px->redirect_rules);
	list_exchange (&px->req_add_new, &px->req_add);
	list_exchange (&px->rsp_add_new, &px->rsp_add);

	LIST_INIT(&px->acl_new);
	LIST_INIT(&px->block_cond_new);
	LIST_INIT(&px->switching_rules_new);
	LIST_INIT(&px->redirect_rules_new);
	LIST_INIT(&px->req_add_new);
	LIST_INIT(&px->rsp_add_new);

	px->req_exp = px->req_exp_new;
	px->rsp_exp = px->rsp_exp_new;
...
/** 
 * clear the new rule of acl, because some error happen
 */
void acl_roll_back(struct proxy *px) 
{
	...
	//clear acl list	
	list_for_each_entry_safe(acl, aclb, &px->acl_new, list) {
		LIST_DEL(&acl->list);
		prune_acl(acl);
		free(acl);
	}
	//clear switching_rule
	list_for_each_entry_safe(rule, ruleb, &px->switching_rules_new, list) {
		LIST_DEL(&rule->list);
	...
	list_for_each_entry_safe(cond, condb, &px->block_cond, list) {
		LIST_DEL(&cond->list);
		prune_acl_cond(cond);
		free(cond);
	}
	...
	//clear redirect_rule
	list_for_each_entry_safe(rdr, rdrb, &px->redirect_rules_new, list) {
		LIST_DEL(&rdr->list);
		if (rdr->cond) {
	...
	for (exp = px->req_exp_new; exp != NULL; ) {
		if (exp->preg) {
	...
	for (exp = px->rsp_exp_new; exp != NULL; ) {
		if (exp->preg) {
			regfree((regex_t *)exp->preg);
			free((regex_t *)exp->preg);
		}
	...
	list_for_each_entry_safe(cwl, cwlb, &px->req_add, list) {
		LIST_DEL(&cwl->list);
		free(cwl->s);
		free(cwl);
	}

	list_for_each_entry_safe(cwl, cwlb, &px->rsp_add, list) {
		LIST_DEL(&cwl->list);
		free(cwl->s);
		free(cwl);
	}
...
/**
 *  clear the acl rule prepare for reload acl rule
 *  include acl list, block list, switch_rule, other need  todo
 *  we clear switching_rules
 */
void clear_acl_rule (struct proxy *px) 
{
	...
	LIST_INIT(&px->acl);
	//clear switching_rule
	list_for_each_entry_safe(rule, ruleb, &px->switching_rules, list) {
		LIST_DEL(&rule->list);
		prune_acl_cond(rule->cond);
		free(rule->cond);
		free(rule);
	...
	LIST_INIT (&px->block_cond);
  //clear redirect_rule
	list_for_each_entry_safe(rdr, rdrb, &px->redirect_rules, list) {
		LIST_DEL(&rdr->list);
		if (rdr->cond) {
			prune_acl_cond(rdr->cond);
			free(rdr->cond);
	...
	LIST_INIT (&px->redirect_rules);

	for (exp = px->req_exp; exp != NULL; ) {
		if (exp->preg) {
			regfree((regex_t *)exp->preg);
			free((regex_t *)exp->preg);
		}
...
/**
 * parse the acl_file, 
 * @reload - false:first read the acl file,true: reload the file
 * @return 
 * 		ERR_FATAL/ERR_ABORT: this error cause haproxy can't continue work, need stop it
 * 		ERR_WARN: 			 haproxy can still work, although this is error
		ERR_NONE: 			 no error
 *
 * use a temp acl list and directive list to store the data, if the 
 * configure file is validate then exchang it to function one
 * supported rule:  block
 *
 */
int acl_load_rule(struct proxy *px, int reload) 
{
	...
	char ack_url[LINESIZE];
	char sequence[ACL_MAX_SEQUENCE_LEN];    /* record the acl(7layer) configuration's sequence */
	...
	// if the file does not exist, we can also use the old conf, only print an log is ok

	if (unlikely(access(trash, R_OK) != 0)) {
		if (unlikely(errno == EACCES)) {
			Alert("ACL£º [%s] has permission problem , please check it", trash);
			return ERR_WARN;
		}

		if (unlikely(mknod (trash, S_IFREG|S_IRWXU|S_IWGRP|S_IRGRP|S_IROTH, 0) != 0))
		{
			Alert("ACL£ºacl_file [%s] didn't exist and can't create the, please check it", trash);
		}
		return ERR_WARN;
	} else {
		...
		f=fopen(trash, "r");
		if (f == NULL) {
			Alert("ACL:reload acl_file %s failed, error:%s", trash, strerror(errno));
			return ERR_WARN;
		}
	}
	...
	while (fgets(thisline, sizeof(thisline), f) != NULL) {
		...
		if (end-line == sizeof(thisline)-1 && *(end-1) != '\n') {
			...
			Alert("parsing [%s:%d]: line too long, limit: %d.\n",
				px->conf.acl_file, linenum, (int)sizeof(thisline)-1);
			err_code |= ERR_ALERT | ERR_FATAL;
		}
		...
		if (*line) {
			...
			*line = '\0';
			...
			Alert("parsing [%s:%d]: line too long, truncating at word %d, position %ld: <%s>.\n",
					px->conf.acl_file, linenum, arg + 1, (long)(args[arg] - thisline + 1), args[arg]);
			err_code |= ERR_ALERT | ERR_FATAL;
			args[arg] = line;
		}
		...
		if (!strcmp(args[0], "acl")) {
			err_code |= process_acl_line (args, px, px->conf.acl_file, linenum, reload); 
			if (err_code & ERR_FATAL)
				goto out;

		} else if (!strcmp(args[0], "use_backend")) {
			err_code |= process_use_backend_line(args, px, px->conf.acl_file, 
												linenum, reload);
			if (err_code & ERR_FATAL)
				goto out;

		} else if (!strcmp(args[0], "redirect")) {
			err_code |= process_redirect_line(args, px, px->conf.acl_file, linenum, reload);
			if (err_code & ERR_FATAL)
				goto out;

		} else if (!strcmp(args[0], "block")) {
			err_code |= process_block_line(args, px, px->conf.acl_file, linenum, reload);
			if (err_code & ERR_FATAL)
				goto out;

		} else if (!strcmp(args[0], "reqdel")) {  /* delete request header from a regex */
			err_code |= create_cond_regex_rule(px->conf.acl_file, linenum, px,
					ACL_DIR_REQ, ACT_REMOVE, 0,
					args[0], args[1], NULL, (const char **)args+2, reload);
			if (err_code & ERR_FATAL)
				goto out;

		/* delete request header from a regex ignoring case */
		} else if (!strcmp(args[0], "reqidel")) {  
			err_code |= create_cond_regex_rule(px->conf.acl_file, linenum, px,
					ACL_DIR_REQ, ACT_REMOVE, REG_ICASE,
					args[0], args[1], NULL, (const char **)args+2, reload);
			if (err_code & ERR_FATAL)
				goto out;
		/* deny a request if a header matches this regex */
		} else if (!strcmp(args[0], "reqdeny")) {  
			err_code |= create_cond_regex_rule(px->conf.acl_file, linenum, px,
		...
		/* pass this header without allowing or denying the request */
			...
			/* allow a request if a header matches this regex */
		} else if (!strcmp(args[0], "reqallow")) {  
			...
			/* tarpit a request if a header matches this regex */
		} else if (!strcmp(args[0], "reqtarpit")) {  
			err_code |= create_cond_regex_rule(px->conf.acl_file, linenum, px,
			...
			/* switch the backend from a regex, respecting case */
		} else if (!strcmp(args[0], "reqsetbe")) { 
			err_code |= create_cond_regex_rule(px->conf.acl_file, linenum, px,
					ACL_DIR_REQ, ACT_SETBE, 0,
					args[0], args[1], args[2], (const char **)args+3, reload);
			if (err_code & ERR_FATAL)
				goto out;

			/* switch the backend from a regex, ignoring case */
		} else if (!strcmp(args[0], "reqisetbe")) { 
			...
			/* replace request header from a regex, ignoring case */
		} else if (!strcmp(args[0], "reqirep")) {  
		...
		/* replace response header from a regex */
		} else if (!strcmp(args[0], "srvexp") || !strcmp(args[0], "rsprep")) {  
			if (*(args[2]) == 0) {
				Alert("parsing [%s:%d] : '%s' expects <search> and <replace> as arguments.\n",
						px->conf.acl_file, linenum, args[0]);
				err_code |= ERR_ALERT | ERR_FATAL;
		...
		/* replace response header from a regex ignoring case */
		} else if (!strcmp(args[0], "rspirep")) {  
			if (*(args[2]) == 0) {
				Alert("parsing [%s:%d] : '%s' expects <search> and <replace> as arguments.\n",
						px->conf.acl_file, linenum, args[0]);
				err_code |= ERR_ALERT | ERR_FATAL;
			...
			/* block response header from a regex */
		} else if (!strcmp(args[0], "rspdeny")) {  
			err_code |= create_cond_regex_rule(px->conf.acl_file, linenum, px,
			...
			/* delete response header from a regex ignoring case */
		} else if (!strcmp(args[0], "rspidel")) {  
			err_code |= create_cond_regex_rule(px->conf.acl_file, linenum, px,
					ACL_DIR_RTR, ACT_REMOVE, REG_ICASE,
					args[0], args[1], NULL, (const char **)args+2, reload);
			if (err_code & ERR_FATAL)
		...
		/* block response header from a regex ignoring case */
		} else if (!strcmp(args[0], "rspideny")) {  
			err_code |= create_cond_regex_rule(px->conf.acl_file, linenum, px,
		...
		} else if (!strcmp(args[0], "sequence")) {  
			if (*(args[1]) == 0) {
				Alert("parsing [%s:%d] : '%s' expects sequence #\n",
						px->conf.acl_file, linenum, args[0]);

				err_code |= ERR_WARN;
				goto out;
			}
			snprintf (sequence, sizeof(sequence), "%s", args[1]);

		} else if (!strcmp (args[0], "ack_url")) {
			if (*(args[1]) == 0) {
				Alert("parsing [%s:%d] : '%s' expects ack_url\n",
						px->conf.acl_file, linenum, args[0]);

				err_code |= ERR_WARN;
				goto out;
			}
			memset (ack_url, 0, sizeof (ack_url));
			snprintf (ack_url, sizeof(ack_url), "%s", args[1]);
		} else {
			Alert ("7layer(acl) rule does not  support for %s\n", args[0]);
			err_code |= ERR_WARN;
		}
...
out: 
	/* 
	 *if new start,it is no need to verify the acl rules, because the system will 
	 * check all conf
	 */
	if (reload){

		if ( !(err_code & ERR_FATAL) && validate_newacl_rule(px) == 0) {
			//replace the acl_rule, no acl rule has function;
			replace_acl_rule(px);
			Alert ("7layer(acl) updated success\n");

			snprintf (acl_sequence, sizeof(acl_sequence), "%s", sequence);
			snprintf(trash, sizeof(trash), "/usr/bin/curl --max-time 5 \"%s&result=SUCCESS&sequence=%s\" &", ack_url, acl_sequence);

		} else {
			// delete the acl_new
			acl_roll_back(px);		
			Alert ("7layer(acl) failed err_code %d", err_code);
			snprintf(trash, sizeof(trash), "/usr/bin/curl \"%s&result=FAILED&sequence=%s\"  &", ack_url, sequence);
		}
...
/*
 * delete space and tab in string
 **/
static int strim_space(char *src)
{
	...
	while(index < str_len)
	{
		//if(isspace((unsigned char)ch[index]))
		if(isblank((unsigned char)ch[index]))
		{
			int start = index;

			while(isspace((unsigned char)ch[++index]));

			//move '\0'
			memmove(ch + start, ch + index, str_len - index + 1);
			index = start + 1;
			continue;
		}
		index++;
		...
		*range++ = 0;
		...
		for (; port <= end; port++) {
			...
			if (ss.ss_family == AF_INET6) {
				...
				if(tcpv6_add_listener(l) != 0)
					goto fail;
			} else {
				((struct sockaddr_in *)(&l->addr))->sin_port = htons(port);
				if(tcpv4_add_listener(l) != 0)
					goto fail;
			}
 ...
 * Return 1 if the warning has been emitted, otherwise 0.
 */
int warnif_rule_after_reqxxx(struct proxy *proxy, const char *file, int line, const char *arg, int reload)
{
	if ((reload == ACL_INIT && proxy->req_exp)
		|| (reload == ACL_RELOAD && proxy->req_exp_new)) {
 ...
 * Return 1 if the warning has been emitted, otherwise 0.
 */
int warnif_rule_after_reqadd(struct proxy *proxy, const char *file, int line, const char *arg, int reload)
{
	if ((reload == ACL_INIT && !LIST_ISEMPTY(&proxy->req_add))
		|| (reload == ACL_RELOAD && !LIST_ISEMPTY(&proxy->req_add_new))) {
 ...
 * Return 1 if the warning has been emitted, otherwise 0.
 */
int warnif_rule_after_redirect(struct proxy *proxy, const char *file, int line, const char *arg, int reload)
{
	if ((reload == ACL_INIT && !LIST_ISEMPTY(&proxy->redirect_rules))
		|| (reload == ACL_RELOAD && !LIST_ISEMPTY(&proxy->redirect_rules_new))) {
...
int warnif_rule_after_use_backend(struct proxy *proxy, const char *file, int line, const char *arg, int reload)
{
	if ((reload == ACL_INIT && !LIST_ISEMPTY(&proxy->switching_rules))
		|| (reload == ACL_RELOAD && !LIST_ISEMPTY(&proxy->switching_rules_new))) {
...
int warnif_misplaced_block(struct proxy *proxy, const char *file, int line, const char *arg, int reload)
{
	return	warnif_rule_after_reqxxx(proxy, file, line, arg, reload) ||
		warnif_rule_after_reqadd(proxy, file, line, arg, reload) ||
		warnif_rule_after_redirect(proxy, file, line, arg, reload) ||
		warnif_rule_after_use_backend(proxy, file, line, arg, reload);
}

/* report a warning if a reqxxx rule is dangerously placed */
int warnif_misplaced_reqxxx(struct proxy *proxy, const char *file, int line, const char *arg, int reload)
{
	return	warnif_rule_after_reqadd(proxy, file, line, arg, reload) ||
		warnif_rule_after_redirect(proxy, file, line, arg, reload) ||
		warnif_rule_after_use_backend(proxy, file, line, arg, reload);
}

/* report a warning if a reqadd rule is dangerously placed */
int warnif_misplaced_reqadd(struct proxy *proxy, const char *file, int line, const char *arg, int reload)
{
	return	warnif_rule_after_redirect(proxy, file, line, arg, reload) ||
		warnif_rule_after_use_backend(proxy, file, line, arg, reload);
}
...
int cfg_parse_global(const char *file, int linenum, char **args, int kwm)
{
	...
	/* end of user/group name handling*/
	...
	else if (!strcmp(args[0], "ulimit-c")) {
		if (global.rlimit_nofile != 0) {
			Alert("parsing [%s:%d] : '%s' already specified. Continuing.\n", file, linenum, args[0]);
			err_code |= ERR_ALERT;
			goto out;
		...
		global.rlimit_core = atol(args[1]);
	}
...
static void init_new_proxy(struct proxy *p)
{
	...
	LIST_INIT(&p->req_add_new);
	LIST_INIT(&p->rsp_add_new);
	LIST_INIT(&p->acl_new);
	LIST_INIT(&p->block_cond_new);
	LIST_INIT(&p->redirect_rules_new);
	LIST_INIT(&p->switching_rules_new);
...
void init_default_instance()
{
	...
	defproxy.defsrv.fastinter = 500;
	defproxy.defsrv.downinter = 500;
	defproxy.defsrv.rise = DEF_RISETIME;
	defproxy.defsrv.fall = DEF_FALLTIME;
	defproxy.defsrv.check_port = 0;
	defproxy.defsrv.maxqueue = 1000;
	defproxy.defsrv.minconn = 0;
	defproxy.defsrv.maxconn = 1000;
	defproxy.defsrv.slowstart = 3;
		...
		const char **cond_start, int reload)
{
	...
	if (cond_start &&
		...
		if ((cond = build_acl_cond(file, line, px, cond_start, reload)) == NULL) {
	...
	if (reload == ACL_RELOAD) 
		err = chain_regex((dir == ACL_DIR_REQ) ? &px->req_exp_new : &px->rsp_exp_new,
	...
	if (dir == ACL_DIR_REQ && warnif_misplaced_reqxxx(px, file, line, cmd, reload))
		err_code |= ERR_WARN;
...
/*
 *  build cookie delete before forward to backend server rules
 *  or delete before forward response to client.
 */
int create_cookie_del_rule(const char *file, int line,
	struct proxy *px, int dir, int action, int flags,
	const char *cmd, const char *reg, const char *repl,
	const char **cond_start)
{
	...
	if (cond_start &&
			(strcmp(*cond_start, "if") == 0 || strcmp(*cond_start, "unless") == 0)) {
		if ((cond = build_acl_cond(file, line, px, cond_start, ACL_INIT)) == NULL) {
			Alert("parsing [%s:%d] : error detected while parsing a '%s' condition.\n",
				file, line, cmd);
			err_code |= ERR_ALERT | ERR_FATAL;
			goto err;
	...
	err = chain_regex((dir == ACL_DIR_REQ) ? &px->req_cookie_del : &px->rsp_cookie_del,
			preg, action, NULL, cond);
	if (repl && err) {
		Alert("parsing [%s:%d] : '%s' : invalid character or unterminated sequence in replacement string near '%c'.\n",
		file, line, cmd, *err);
		err_code |= ERR_ALERT | ERR_FATAL;
...
int cfg_parse_listen(const char *file, int linenum, char **args, int kwm)
{
	...
	if (rc != PR_CAP_NONE) {  /* new proxy */			
		if (!*args[1] ) {
			Alert("parsing [%s:%d] : '%s' expects an <id> argument and\n"
				...
				file, linenum, args[0]);
			if ( asprintf(&id, "%s_%d", args[0], linenum) == -1 ) {
		...
		for (curproxy = proxy; curproxy != NULL; curproxy = curproxy->next) {
			...
			if (!strcmp(curproxy->id, id) &&
				...
				Warning("Parsing [%s:%d]: %s '%s' has same name as another %s (declared at %s:%d).\n",
					file, linenum, proxy_cap_str(rc), id, proxy_type_str(curproxy),
		...
		curproxy->cap = rc;
		curproxy->lbprm.reinit = NULL;
		curproxy->unused_srv = NULL;
		...
		if (curproxy->cap & PR_CAP_FE) {
			...
			//no server file
			curproxy->conf.acl_file = NULL;
			curproxy->conf.backend_server_file = NULL;
			curproxy->conf.inotify_fd = -1;
		}
		...
		if (curproxy->cap & PR_CAP_BE) {
			...
			//server list file,default filename:backend id
			curproxy->conf.backend_server_file = strdup(args[1]);
			curproxy->conf.inotify_fd = -1;
		}
	...
	if (!strcmp(args[0], "bind")) {  /* new listen addresses */
		...
		if ((cond = build_acl_cond(file, linenum, curproxy, (const char **)args + 1, ACL_INIT)) == NULL) {
			Alert("parsing [%s:%d] : error detected while parsing blocking condition.\n",
				file, linenum);
			err_code |= ERR_ALERT | ERR_FATAL;
			goto out;
		...
		LIST_ADDQ(&curproxy->block_cond, &cond->list);
		warnif_misplaced_block(curproxy, file, linenum, args[0], ACL_INIT);
	}
	else if (!strcmp(args[0], "redirect")) {
		struct redirect_rule *rule;
		int cur_arg;
		...
		while (*(args[cur_arg])) {
			if (!strcmp(args[cur_arg], "location")) {
				...
				flags |= REDIRECT_FLAG_APPEND_SLASH;
					...
					strcmp(args[cur_arg], "unless") == 0) {
				cond = build_acl_cond(file, linenum, curproxy, (const char **)args + cur_arg, ACL_INIT);
				if (!cond) {
					Alert("parsing [%s:%d] : '%s': error detected while parsing redirect condition.\n",
						file, linenum, args[0]);
					err_code |= ERR_ALERT | ERR_FATAL;
		...
		LIST_INIT(&rule->list);
		LIST_ADDQ(&curproxy->redirect_rules, &rule->list);
		warnif_rule_after_use_backend(curproxy, file, linenum, args[0], ACL_INIT);
	}
	else if (!strcmp(args[0], "use_backend")) {
		struct switching_rule *rule;
		...
		if ((cond = build_acl_cond(file, linenum, curproxy, (const char **)args + 2, ACL_INIT)) == NULL) {
			Alert("parsing [%s:%d] : error detected while parsing switching rule.\n",
				file, linenum);
			err_code |= ERR_ALERT | ERR_FATAL;
			goto out;
		...
		if (strcmp(args[myidx], "if") == 0 || strcmp(args[myidx], "unless") == 0) {
			if ((cond = build_acl_cond(file, linenum, curproxy, (const char **)args + myidx, ACL_INIT)) == NULL) {
...
stats_error_parsing:
	...
	else if (!strcmp(args[0], "server_file")) {
		if (*(args[1]) == '\0') {
		...
		if(curproxy->conf.backend_server_file) {
			free(curproxy->conf.backend_server_file);
			curproxy->conf.backend_server_file = NULL;
		}
		curproxy->conf.backend_server_file = strdup(args[1]);
		if(NULL == curproxy->conf.backend_server_file) {
			Alert("parsing [%s:%d]: out of memory.\n", file, linenum);
			err_code |= ERR_ALERT | ERR_ABORT;
			goto out;
		}
	...
	else if (!strcmp(args[0], "acl_file")) {

		if (*(args[1]) == '\0') {
			Alert("parsing [%s:%d]: '%s' expects an acl file name.\n",
					file, linenum, args[0]);
			err_code |= ERR_ALERT | ERR_FATAL;
			goto out;
		}
		...
		if(curproxy->conf.acl_file) {
			free(curproxy->conf.acl_file);
			curproxy->conf.acl_file = NULL;
		}
		curproxy->conf.acl_file = strdup(args[1]);
		if(NULL == curproxy->conf.acl_file) {
			Alert("parsing [%s:%d]: out of memory.\n", file, linenum);
			err_code |= ERR_ALERT | ERR_ABORT;
			goto out;
		}
		...
		/*option 3*/
		for (optnum = 0; cfg_opts3[optnum].name; optnum++) {
			if (!strcmp(args[1], cfg_opts3[optnum].name)) {
				if (warnifnotcap(curproxy, cfg_opts3[optnum].cap, file, linenum, args[1], NULL)) {
					err_code |= ERR_WARN;
					goto out;
				}

				curproxy->no_options3 &= ~cfg_opts3[optnum].val;
				curproxy->options3    &= ~cfg_opts3[optnum].val;
		...
		if (!strcmp(args[1], "httplog")) {
			...
			/*
			 * don't re-add forwardfor by default
			 */
			...
			while (*(args[cur_arg])) {
				if (!strcmp(args[cur_arg], "except")) {
					...
					curproxy->options2 &= ~PR_O2_FF_ALWAYS;
					cur_arg += 1;
				} else if (!strcmp(args[cur_arg], "if-always")) {
					curproxy->options2 |= PR_O2_FF_ALWAYS;
					cur_arg += 1;
				}
				else {
		...
		if (!defsrv) {
			...
			if ((newsrv = (struct server *)pool_alloc2(pool2_server)) == NULL) {
				Alert("parsing [%s:%d] : out of memory.\n", file, linenum);
				err_code |= ERR_ALERT | ERR_ABORT;
				goto out;
			}
			memset(newsrv, 0, sizeof(struct server));
		...
		if (!defsrv) {
			...
			err_code |= create_cond_regex_rule(file, linenum, curproxy,
					ACL_DIR_REQ, ACT_REPLACE, 0,
					args[0], args[1], args[2], (const char **)args+3, ACL_INIT);
			if (err_code & ERR_FATAL)
				goto out;
		}
		else if (!strcmp(args[0], "reqdel")) {  /* delete request header from a regex */
			err_code |= create_cond_regex_rule(file, linenum, curproxy,
					ACL_DIR_REQ, ACT_REMOVE, 0,
					args[0], args[1], NULL, (const char **)args+2, ACL_INIT);
			if (err_code & ERR_FATAL)
				goto out;
		}
		...
		/*clear cookie before forward to backend server, modify by sixing*/
		else if(!strcmp(args[0], "clear-cli-cookie"))
		{
			if (warnifnotcap(curproxy, PR_CAP_BE, file, linenum, args[0], NULL))
				err_code |= ERR_WARN;
			err_code |= create_cookie_del_rule(file, linenum, curproxy,
					ACL_DIR_REQ, ACT_REMOVE, REG_ICASE,
					args[0], args[1], NULL, (const char **)args+2);
			if (err_code & ERR_FATAL)
				goto out;        
		}
		else if(!strcmp(args[0], "clear-srv-cookie"))
		{
			if (warnifnotcap(curproxy, PR_CAP_BE, file, linenum, args[0], NULL))
				err_code |= ERR_WARN;
			err_code |= create_cookie_del_rule(file, linenum, curproxy,
			...
			warnif_misplaced_reqadd(curproxy, file, linenum, args[0], ACL_INIT);
		}
		...
		else if(!strcmp(args[0], "hdrname2xforward"))
		{
			if (warnifnotcap(curproxy, PR_CAP_FE , file, linenum, args[0], NULL))
				err_code |= ERR_WARN;

			if (*(args[1]) == 0) {
				Alert("parsing [%s:%d] : <%s> expects hdr-name as arguments.\n", file, linenum, args[0]);
				err_code |= ERR_ALERT | ERR_FATAL;
				goto out;
			}
			curproxy->hdrname2xforward = strdup(args[1]);
			if(NULL == curproxy->hdrname2xforward)
			{
				Alert("Duplicate hdrname failed!\n");
				err_code |= ERR_ALERT | ERR_FATAL;
				goto out;
			}
			curproxy->hdrname2xforward_len = strlen(args[1]);
		}
		else {
			struct cfg_kw_list *kwl;
			int index;
...
int readcfgfile(const char *file)
{
	...
	while (fgets(thisline, sizeof(thisline), f) != NULL) {
		...
		switch (confsect) {
			case CFG_LISTEN:
				pro_err_code = 0;
				pro_err_code = cfg_parse_listen(file, linenum, args, kwm);
				err_code |= pro_err_code;

				/* encounter a configure error in current proxy.*/
				if(pro_err_code & ERR_CODE) { 
					struct proxy *curproxy = proxy;
					curproxy->conf.flags |= PROXY_CONFIG_INVALID;
					/*
					 *  disable frontend proxy!!
					 */
					if(!strcmp(cursection, "frontend") || !strcmp(cursection, "listen")) {
						curproxy->state = PR_STSTOPPED;
					}
				}
				break;
...
int check_config_validity()
{
	...
	while (curproxy != NULL) {
		...
		if ((curproxy->cap & PR_CAP_BE) && (curproxy->mode != PR_MODE_HEALTH)) {
			if (curproxy->lbprm.algo & BE_LB_KIND) {
				if (curproxy->options & PR_O_TRANSP) {
					Alert("config : %s '%s' cannot use both transparent and balance mode.fall to balance mode\n",
							proxy_type_str(curproxy), curproxy->id);
					cfgerr++;
					curproxy->options &= ~PR_O_TRANSP;
				}
		...
		if (curproxy->defbe.name) {
			...
			if (!target) {
				Alert("Proxy '%s': unable to find required default_backend: '%s',will disable it.\n",
		...
		list_for_each_entry(mrule, &curproxy->storersp_rules, list) {
			...
			if (!target) {
				Alert("Proxy '%s': unable to find store table '%s', and will disable storersp rules\n",
						curproxy->id, mrule->table.name);
				curproxy->be_rsp_ana &= ~AN_RES_STORE_RULES;
			}
			else if (target->table.size == 0) {
				Alert("Proxy '%s': stick-table '%s' used but not configured, and will disable storersp rules\n",
						curproxy->id, mrule->table.name ? mrule->table.name : curproxy->id);
				curproxy->be_rsp_ana &= ~AN_RES_STORE_RULES;
			}
			else if (pattern_notusable_key(mrule->expr, target->table.type)) {
				Alert("Proxy '%s': type of pattern not usable with type of stick-table '%s', and will disable storersp rules\n",
						curproxy->id, mrule->table.name ? mrule->table.name : curproxy->id);
				curproxy->be_rsp_ana &= ~AN_RES_STORE_RULES;
			}
			else {
		...
		while (newsrv != NULL) {
			if (newsrv->minconn > newsrv->maxconn) {
				...
				Alert("config : %s '%s' : fullconn is mandatory when minconn is set on a server.\n",
						proxy_type_str(curproxy), curproxy->id);
				cfgerr++;
				curproxy->fullconn = newsrv->maxconn;
			}
...
/*
 * read server list from backend file
 */
int read_server_file(struct proxy* px)
{
	FILE* fp = NULL;
	char thisline[LINESIZE];
	char *file = px->conf.backend_server_file;
	...
	memset(trash, 0, sizeof(trash));
	snprintf(trash, sizeof(trash)/sizeof(trash[0]) - 1, "%s/%s", BACKEND_FILE_PATH, file);

	if (stat(trash,&f_stat)) {
		fprintf(stderr, "file %s does't exist, create it!\n", px->conf.backend_server_file);
		ret = -1;

		//create if it dons't exist
		fp = fopen(trash, "a+");
		goto exit;
	}
	else 
	...
	if (fp == NULL) {
		ret = -1;
		fprintf(stderr, "open file %s failed, error:%s", file, strerror(errno));
		goto exit;
	}
	...
	while (fgets(thisline, sizeof(thisline), fp) != NULL) {
		char *line = thisline;
		int nargs = 0;
		//ip,port,id,weight,maxconn,check_inter,fall
		char *args[6], *beg;
		struct sockaddr_in *sk = NULL;
		int port, weight, maxconn,check_inter,fall;

		thisline[LINESIZE -1] = '\0';/*truncate string*/
		/* skip leading spaces */
		while (isspace((unsigned char)*line))
			line++;
		if (*line == '#' || *line == '\n' || *line == '\r') {
			/* end of string, skip this line*/
			*line = 0;
			continue;
		}
		...
		/*
		 * ip, port mast provide,:-)
		 */
		//if(nargs < 5 || !*args[0] || !*args[1] ) {
		if(nargs < 5 || !*args[0] ) {
			Alert("parse [%s:%d]: invaid format!\n", file,  line_num);
			continue;
		}

		/*
		 * might block if use domain name here
		 */
		sk = str2sa(args[0]);
		if(!sk || ((sk->sin_addr).s_addr == INADDR_ANY )) {
			Alert("parse [%s:%d]: Unknown host in `%s`\n", file, line_num, args[0]);
			continue;
		}

#define CHECK_SRV_PARA(para,args) do{\
	if((*args) != '\0') \
	(para) = atol(args);\
	else \
	(para) = 0; \
	if((para) < 0) { \
		Alert("parse [%s:%d] expects an integer argument\n", file, line_num); \
		continue;\
	}\
}while(0)
	CHECK_SRV_PARA(port, args[1]);
	CHECK_SRV_PARA(weight, args[2]);
	CHECK_SRV_PARA(maxconn, args[3]);
	CHECK_SRV_PARA(check_inter, args[4]);
	CHECK_SRV_PARA(fall, args[5]);

	//add a server
	if ((srv = (struct server*) pool_alloc2(pool2_server)) == NULL) {
		Alert("parsing [%s:%d] : out of memory.\n", px->conf.backend_server_file, line_num);
		ret = -1;
		goto exit;
	}
	...
	srv->proxy = px;
	srv->conf.file = file;
	srv->conf.line = line_num;

	LIST_INIT(&srv->pendconns);
	srv->state = SRV_RUNNING; /* early server setup */
	srv->last_change = now.tv_sec;

	/*assign server id ,it's useless*/
	asprintf(&(srv->id), "inst_%d", line_num);
	srv->puid = line_num * 1000;

	/* assign cookie*/
	asprintf(&(srv->cookie), "usrmdinst_%d",line_num);
	srv->cklen = strlen(srv->cookie);

	//assign server ip address and port info
	srv->addr = *sk;
	if (0 == port)
		srv->state |= SRV_MAPPORTS;
	else
		srv->addr.sin_port = htons(port);

	srv->check_port  = px->defsrv.check_port;
	srv->inter       = px->defsrv.inter;
	srv->fastinter   = px->defsrv.fastinter;
	srv->downinter   = px->defsrv.downinter;
	srv->rise        = px->defsrv.rise;
	srv->fall        = px->defsrv.fall;
	srv->maxqueue    = px->defsrv.maxqueue;
	srv->minconn     = px->defsrv.minconn;
	srv->maxconn     = px->defsrv.maxconn;
	srv->slowstart   = px->defsrv.slowstart;
	srv->onerror     = px->defsrv.onerror;
	srv->consecutive_errors_limit
	= px->defsrv.consecutive_errors_limit;
	srv->uweight = srv->iweight
	= px->defsrv.iweight;

	srv->curfd = -1;     /* no health-check in progress */
	srv->health = srv->rise;  /* up, but will fall down at first failure */
	srv->check_data = NULL;

#define ASSIGN_SRV_ATTR(args, attr) do{ \
	if((args) > 0) \
	(attr) = (args);\
}while(0)

	ASSIGN_SRV_ATTR(check_inter, srv->inter);
	ASSIGN_SRV_ATTR(weight, srv->uweight);
	ASSIGN_SRV_ATTR(weight, srv->iweight);
	ASSIGN_SRV_ATTR(maxconn, srv->maxconn);
	ASSIGN_SRV_ATTR(fall, srv->fall);

	srv->conf.id.key = srv->puid;
	node = eb32_lookup(&px->conf.used_server_id, srv->puid);
	if(node) {
		struct server *target = container_of(node, struct server, conf.id);
		send_log(px, LOG_WARNING, 
				"parsing [%s:%d]: server %s reuses same custom id as server %s (declared at %s:%d).\n",
				px->conf.backend_server_file, line_num, srv->id, target->id, target->conf.file, 
				target->conf.line);
		srv->conf.id.key = srv->puid = line_num * 1000;
		while(eb32_lookup(&px->conf.used_server_id, srv->puid)) {
			srv->conf.id.key = srv->puid = rand() % 5000;
		}
	}
	eb32_insert(&px->conf.used_server_id, &srv->conf.id);

	/*disable server check in port map mode and check inter is zero?*/
	if (!(srv->state & SRV_MAPPORTS) && srv->inter != 0)
	{
		srv->check_port = atol(args[1]);
		srv->check_status = HCHK_STATUS_INI;
		srv->state |= SRV_CHECKED;

		/* Allocate buffer for partial check results... */
		if ((srv->check_data = calloc(global.tune.chksize, sizeof(char))) == NULL) {
			send_log(px, LOG_WARNING,"parsing [%s:%d] : out of memory while allocating check buffer.\n", 
					px->conf.backend_server_file, line_num);
			ret = -1;
			goto exit;
		}
