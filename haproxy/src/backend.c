...
struct server *get_server_uh(struct proxy *px, char *uri, int uri_len)
{
	...
	/*
	 * hash-type file-name or uri
	 */
	if(px->options3 & PR_O3_HASH_FILENAME)
	{
		/*
		 * trim backspace
		 */
		while((hash_para_len >= 0) && (uri[hash_para_len] == ' '))
			--hash_para_len;

		if(uri[hash_para_len] != '/')
		{
			int len = 1;

			c = hash_para_len -1;
			while((len <= hash_para_len -1) && (uri[c] != '/'))
			{
				++len;
				--c;
			}
