...
void sess_establish(struct session *s, struct stream_interface *si)
{
	...
	if(likely(tv_iseq(&((s->logs).tv_connect), &now))) {
		s->logs.t_l4_delay = 0;
	} else {
		s->logs.t_l4_delay = tv_ms_elapsed(&((s->logs).tv_connect), &now);
	}
...
void sess_update_stream_int(struct session *s, struct stream_interface *si)
{
	...
	if (si->state == SI_ST_ASS) {
		...
		if (tv_iszero(&((s->logs).tv_connect)) == 1) {
			s->logs.tv_connect = now;
			s->logs.t_l7_delay = 
				(RDTSC() - s->logs.rdtsc_request_header) * 1000000 / cpu_frequency;
		}
 ...
 resync_response:
		...
		s->logs.t_srv_delay != -1)
	{
		...
		if (s->fe->mode == PR_MODE_HTTP) {
			// because the t_l7_delay is us, change to ms
			t = s->logs.t_l7_delay / 1000;
			if( t >= 0 && t < 3 )
