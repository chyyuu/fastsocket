...
struct pool_head *pool2_server;

int init_server(void)
{
	//pool2_server = create_pool("server", sizeof(struct server), MEM_F_SHARED);
	pool2_server = create_pool("server", sizeof(struct server), 0);
	return pool2_server != NULL;
}
