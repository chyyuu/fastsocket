...
{
	int index = 0;
	struct sockaddr_un* p_unsock_addr = NULL, *p_tmp_unaddr = NULL;
	...
	if (!strcmp(args[0], "socket")) {
		...
		p_unsock_addr = (struct sockaddr_un*)&global.stats_sock.addr;        
		global.p_stats_sock = (struct listener*)calloc(global.nbproc, sizeof(struct listener));  
		if(NULL == global.p_stats_sock)
		{
			snprintf(err, errlen, "Alloc memory for stat sock failed!\n");
			return -1;
		}
		for(index = 0; index < global.nbproc; index++)
		{
			global.p_stats_sock[index] = global.stats_sock;
			p_tmp_unaddr = (struct sockaddr_un*)&global.p_stats_sock[index].addr;
			snprintf(p_tmp_unaddr->sun_path, 
					sizeof(p_unsock_addr->sun_path),"%s_%d", 
					p_unsock_addr->sun_path, index+1);

			/*add to the global.proxy listener list*/
			global.p_stats_sock[index].next  = global.stats_fe->listen;
			global.stats_fe->listen = &global.p_stats_sock[index];

			/*register listener*/
			uxst_add_listener(&global.p_stats_sock[index]);
			global.maxsock++;
		...
		if (si->st0 == STAT_CLI_INIT) {
			...
			case STAT_CLI_O_ERR:	/* errors dump */
				if (stats_dump_errors_to_buffer(s, res))
					si->st0 = STAT_CLI_PROMPT;
				break;
			case STAT_CLI_O_DELAY:
				if(stats_dump_delay_statics_to_buffer(s, res))
					si->st0 = STAT_CLI_PROMPT;
				break;
...
/*
 * This function is called to send delay statics to buffer.
 * it will dump all delay statics to output buffer owned by all proxy
 * frontend proxy only owns l7_delay statics, and backend proxy owns l4_delay( time of connect to backend server)
 * and server response delay, response data transfer time.
 */
int stats_dump_delay_statics_to_buffer(struct session *s, struct buffer *rep)
{
	struct proxy *px;
	struct server *srv;
	struct chunk msg;
		...
		if (s->data_ctx.stats.flags & STAT_SHOW_STAT) {
			while (s->data_ctx.stats.px) {
				...
				case DATA_ST_PX_BE:
					...
					if (px->state != PR_STSTOPPED) {					
						if (px->cap & PR_CAP_FE) {
							p = &px->counters.delay.req_delay.l7_delay[0];
							chunk_printf(&msg, "%s,FRONTEND,%lld,%lld,%lld,%lld\n", px->id, 
											p[0], p[1], p[2], p[3]);
						} else {
							if(px->cap & PR_CAP_BE) {
								/* output l4 connect delay statics */
								chunk_printf(&msg, "%s,BACKEND,", px->id);
								p = &px->counters.delay.res_delay.l4_conn_delay[0];
								chunk_printf(&msg, "%lld,%lld,%lld,", p[0], p[1], p[2]);

								/* output srv response delay */
								p = &px->counters.delay.res_delay.res_delay[0];
								chunk_printf(&msg, "%lld,%lld,%lld,%lld,", 
									p[0], p[1], p[2], p[3]);

								/* output response data transfer delay */
								p = &px->counters.delay.res_delay.res_data_delay[0];
								chunk_printf(&msg, "%lld,%lld,%lld,%lld,%lld", 
										p[0], p[1], p[2], p[3],p[4]);
								chunk_printf(&msg, "\n");
							}
						}
				...
				case DATA_ST_PX_SV:
					while (s->data_ctx.stats.sv) {
						/* output l4 connect delay statics */
						srv = s->data_ctx.stats.sv;
						chunk_printf(&msg, "%s/%s:%d,SERVER,",  (srv->proxy)->id,
											inet_ntoa((srv->addr).sin_addr), 
											ntohs((srv->addr).sin_port));
						p = &srv->counters.delay.res_delay.l4_conn_delay[0];
						chunk_printf(&msg, "%lld,%lld,%lld,", p[0], p[1], p[2]);

						/* output srv response delay */
						p = &srv->counters.delay.res_delay.res_delay[0];
						chunk_printf(&msg, "%lld,%lld,%lld,%lld,", 
							p[0], p[1], p[2], p[3]);

						/* output response data transfer delay */
						p = &srv->counters.delay.res_delay.res_data_delay[0];
						chunk_printf(&msg, "%lld,%lld,%lld,%lld,%lld", 
								p[0], p[1], p[2], p[3],p[4]);
						chunk_printf(&msg, "\n");
		...
		if ((px->cap & PR_CAP_FE) &&
			...
			if (!(s->data_ctx.stats.flags & STAT_FMT_CSV)) {
				...
				if (px->mode == PR_MODE_HTTP) {
					...
					/*output 4xx,5xx detail info*/
					for(i = 0; i < 9; i++)
						chunk_printf(&msg, "40%d=%lld,", i, px->counters.fe.http.rsp_4xx[i]);
					for(i = 0; i < 9; i++)
						chunk_printf(&msg, "50%d=%lld,", i, px->counters.fe.http.rsp_5xx[i]);
				...
				if (px->mode == PR_MODE_HTTP) {
					...
					/*output 4xx,5xx detail info*/
					for(i = 0; i < 5; i++)
						chunk_printf(&msg, "%lld,", px->counters.fe.http.rsp_4xx[i]);

					for(i = 0; i < 5; i++)
						chunk_printf(&msg, "%lld,", px->counters.fe.http.rsp_5xx[i]);
		...
		for (; s->data_ctx.stats.sv != NULL; s->data_ctx.stats.sv = sv->next) {
			...
			if (!(s->data_ctx.stats.flags & STAT_FMT_CSV)) {
				...
				if (px->mode == PR_MODE_HTTP) {
					...
					/*output 4xx,5xx detail info*/
					for(i = 0; i < 9; i++)
						chunk_printf(&msg, "40%d=%lld,", i, sv->counters.p.http.rsp_4xx[i]);

					for(i = 0; i < 9; i++)
						chunk_printf(&msg, "50%d=%lld,", i, sv->counters.p.http.rsp_5xx[i]);
				...
				if (px->mode == PR_MODE_HTTP) {
					...
					/*output 4xx,5xx detail info*/
					for(i = 0; i < 5; i++)
						chunk_printf(&msg, "%lld,", sv->counters.p.http.rsp_4xx[i]);

					for(i = 0; i < 5; i++)
						chunk_printf(&msg, "%lld,", sv->counters.p.http.rsp_5xx[i]);
		...
		if ((px->cap & PR_CAP_BE) &&
			...
			if (!(s->data_ctx.stats.flags & STAT_FMT_CSV)) {
				...
				if (px->mode == PR_MODE_HTTP) {
					...
					/*output 4xx,5xx detail info*/
					for(i = 0; i < 9; i++)
						chunk_printf(&msg, "40%d=%lld,", i, px->counters.be.http.rsp_4xx[i]);

					for(i = 0; i < 9; i++)
						chunk_printf(&msg, "50%d=%lld,", i, px->counters.be.http.rsp_5xx[i]);
				...
				if (px->mode == PR_MODE_HTTP) {
					...
					/*output 4xx,5xx detail info*/
					for(i = 0; i < 5; i++)
						chunk_printf(&msg, "%lld,", px->counters.be.http.rsp_4xx[i]);

					for(i = 0; i < 5; i++)
						chunk_printf(&msg, "%lld,", px->counters.be.http.rsp_5xx[i]);
