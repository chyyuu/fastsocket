...
extern struct  timeval tv_stop;
extern int shut_down_now;
/*
...
void maintain_proxies(int *next)
{
	...
	if (stopping) {
		...
		/*
		 * shut down it when 1day elapsed . 
		 */
		if(tv_iszero(&tv_stop) == 0) {
			down_time = tv_ms_elapsed(&tv_stop, &now);
			/*
			 * 24h * 60min * 60sec
			 */
			if(down_time / 1000 > 86400) {
