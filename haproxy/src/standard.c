...
struct sockaddr_in *str2sa(char *str)
{
	...
	strncpy(str_addr, str, sizeof(str_addr));

	if ((c = strrchr(str_addr,':')) != NULL) {
		*c++ = '\0';
		port = atol(c);
	}
	else
	...
	if (*str_addr == '*' || *str_addr == '\0') { /* INADDR_ANY */
		sa.sin_addr.s_addr = INADDR_ANY;
	}
	else if (!inet_pton(AF_INET, str_addr, &sa.sin_addr)) {
