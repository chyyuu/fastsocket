...
/*
 * compare tcp listener's ip and port, duplicate listener will bind error
 * and haproxy will exit.-(:
 **/
static int tcpv4_listener_comp(struct listener *from, struct listener *to)
{
	int ret = 0;

	ret = ((struct sockaddr_in *)(&from->addr))->sin_port == 
			((struct sockaddr_in *)(&to->addr))->sin_port;

	ret = ret && ((((struct sockaddr_in *)(&from->addr))->sin_addr.s_addr) == 
			((struct sockaddr_in *)(&to->addr))->sin_addr.s_addr);
 ...
 */
int tcpv4_add_listener(struct listener *listener)
{
	struct listener *li;
	...
	{
		/* found a same listener, Alert and exit?
		 **/
		if(tcpv4_listener_comp(li, listener)) {
			struct sockaddr_in *ss = (struct sockaddr_in *)&li->addr;
			
			Alert("Found duplicate bind address [%s:%d]\n", 
 ...
 * and haproxy will exit.-(:
 **/
static int tcpv6_listener_comp(struct listener *from, struct listener *to)
{
	int ret = 0;

	ret = ((struct sockaddr_in6 *)(&from->addr))->sin6_port == 
			((struct sockaddr_in6 *)(&to->addr))->sin6_port;

	ret = ret && !memcmp((struct sockaddr_in6 *)(&from->addr),
				(struct sockaddr_in6 *)(&to->addr), sizeof(struct sockaddr_in6));
 ...
 * listeners is updated. This is the function to use to add a new listener.
 */
int tcpv6_add_listener(struct listener *listener)
{
	struct listener *li;
	...
	list_for_each_entry(li, &proto_tcpv6.listeners, proto_list)
	{
		/* found a same listener, Alert and exit?
		 **/
		if(tcpv6_listener_comp(li, listener)) {
			struct sockaddr_in6 *ss = (struct sockaddr_in6 *)&li->addr;
			char ipv6_addr[INET6_ADDRSTRLEN +1] = "\0";

			inet_ntop(AF_INET6, &ss->sin6_addr, ipv6_addr, sizeof(ipv6_addr));
			Alert("Found duplicate bind address [%s:%d]\n", 
					ipv6_addr, ntohs(ss->sin6_port));
			     ...
			     struct proxy *defpx, char *err, int errlen)
{
	...
	if (!strcmp(args[1], "content")) {
		...
		if (strcmp(args[3], "if") == 0 || strcmp(args[3], "unless") == 0) {
			if ((cond = build_acl_cond(NULL, 0, curpx, (const char **)args+3, ACL_INIT)) == NULL) {
				retlen = snprintf(err, errlen,
