...
struct inotify_info *inotify_info_tab = NULL;
...
void display_version()
{
	...
	printf ("SVN vrsion " SVN_VERSION "\n"); 
	printf ("Compile at " COMPILE_DATE "\n");
	printf("Copyright 2000-2011 Willy Tarreau <w@1wt.eu>\n\n");
	printf("ADD:\n");
	printf("    1.Auto reload backend server file\n");
	printf("    2.export lb-name in response header\n");
	printf("    3.output 4xx,5xx detail info in cvs or html format\n");
	printf("    4.clear-cli-cookie to delete defined cookie before forward to server\n");
	printf("    5.cook_val layer switch acl\n");
	printf("    6.health check ok even server 4xx response\n");
	printf("    7.output each haproxy running info via unix socket\n");
	printf("    8.only hash filename when use balance uri\n");
	printf("    9.address duplicate detect,and ignor listenner bind error\n");
	printf("    10.output backtrace info to logserver when error occured\n");
	printf("    11.ignore sub proxy configure error when start haproxy\n");
	printf("    12.import change http request hdr-name to X-Forwarded-For\n");
	printf("    13.export L4,L7,SRV delay via SINA-TS in HTTP response header\n");
	return;
}
...
long get_cpu_frequency()
{
    ...
    // the cpu MHz should appear in 8th line
    while (fgets(line, sizeof(line), f) != NULL) {
        if (strstr (line, "cpu MHz")!= NULL) break;
...
void sig_dump_state(int sig)
{
	...
	for( p = proxy; p != NULL; p = p->next) {
		if (( p->cap & PR_CAP_BE ) && ( p->options3 & PR_O3_SERVER_FROM_FILE )) {
			/*
			* read server list from file at /etc/haproxy/backend.d/proxy_name
			*/
			reload_backend_server(p);
		}
	}
#endif
}
...
/*
 * Dump call stack debug infomation, 
 * use this function may fall into busy loop
 */
void dump_call_stack(int sig)
{
	struct sigaction      sa;
	size_t                size;

	//reset signal handler to DEF
	sa.sa_handler = SIG_DFL;
	sigemptyset(&sa.sa_mask);
	if (sigaction(sig, &sa, NULL) == -1) {
		send_log(global.stats_fe, LOG_WARNING, "Reset signal %d handler to DFL error!, error=%s\n",
				sig, strerror(errno));
	}

	memset(trash, 0, sizeof(trash));
	size = backtrace(&trash[0], sizeof(trash));

	//write to /etc/haproxy/pid.stack
	//
	{
#if 0
		int fd = open("/etc/haproxy/backtrace.log", O_APPEND | O_CREAT | O_SYNC);
		if(fd > 0)
		{
			int ret = 0;
			backtrace_symbols_fd(trash, size, fd);
			ret = write(fd, "backtracelog",10 );
			close(fd);
		}
#endif
		char **symb = backtrace_symbols(trash, size);
		if(symb != NULL) {
			int i = 0;
			for(i = 0 ; i < size; i++) {
				send_log(NULL, LOG_ALERT, "backtrace:%s", symb[i]);
			}
			free(symb);
		}
	}

	kill(getpid(),sig);//forward this signal
...
void init(int argc, char **argv)
{
	...
	a2base64(hostname, strlen(hostname), lb_name, sizeof(lb_name));
	...
	for( px = proxy; px != NULL; px = px->next) {
		if (( px->cap & PR_CAP_FE ) && ( px->options3 & PR_O3_ACL_FROM_FILE )) {
			/*
			 * read acl file at the position that configure file set 
			 */
			acl_load_rule(px, ACL_INIT);
		}
	}
	...
	/* exit haproxy when only encounter an abort error
	 * usually only memory error will return an abort error. 
	 **/
	if(err_code & ERR_ABORT)
		exit(1);
	...
	if (global.mode & MODE_CHECK) {
		if (err_code & (ERR_FATAL | ERR_ABORT)) {
			qfprintf(stdout, "Configuration file isn't valid\n");
			exit(1);
		} else {
...
#if 0
	/*
	 * init inotify info list,used to record watchfd's proxy
	 */
	inotify_info_tab = (struct inotify_info*)calloc(1, sizeof(struct inotify_info) * (global.maxsock));
	for (i = 0; i < global.maxsock; i++) {
		inotify_info_tab[i].wd = -1;
		inotify_info_tab[i].px = NULL;
	}
#endif
...
void deinit(void)
{
	...
	while (p) {
		...
		if(NULL != p->hdrname2xforward)
			free(p->hdrname2xforward);
...
void run_poll_loop()
{
	...
	while (1) {
		...
		if ((!actconn && listeners == 0) || (shut_down_now == 1))
			break;
...
int main(int argc, char **argv)
{
	...
	if (cpu_frequency <= 0) {
		printf ("ERROR: get cpu frequency failed, please check up!\n");
		return -1;
	}
...
#endif
	/*
 	 * conflict with core dump! 
 	 */
	//signal_register(SIGSEGV, dump_call_stack); 
	...
	if (global.rlimit_nofile) {
		...
		limit.rlim_max = global.rlimit_nofile + 1;
	...
	if ((err & ~ERR_WARN) != ERR_NONE) {
		...
		exit(1);
#endif
		Alert("Start proxy error,-:(\n");

		/*check bind address
		*/
		list_for_each_entry(li, &proto_tcpv4.listeners, proto_list) {
			if(li->state == LI_LISTEN) 
				enlis++;
		}
		list_for_each_entry(li, &proto_tcpv6.listeners, proto_list) {
			if(li->state == LI_LISTEN) 
				enlis++;
		}

		if (enlis == 0) {
			Alert("No proxy listener address bind success, Haproxy will exit!!\n");
			exit(1);
		}
	}
	...
	if ((protocol_bind_all() & ~ERR_WARN) != ERR_NONE) {
		...
		exit(1);
#endif
		Alert("Start proxy error,please check all the listener bind successfully\n");
	}
	...
	if (global.mode & MODE_DAEMON) {
		...
		if( NULL != global.p_stats_sock) {
			for (index = 0; index < global.nbproc; index++) {
				if(index == relative_pid -1)
					continue;
				l = &global.p_stats_sock[index];
				unbind_listener(l);
	...
	//DUMP CORE
	if (1) {
		//limit.rlim_cur = global.rlimit_core;
	...
	if (prctl(PR_SET_DUMPABLE, 1, 0, 0, 0) == -1) {
		Alert("prctl(PR_SET_DUMPABLE) failed");
		send_log(NULL, LOG_ALERT, "prctl(PR_SET_DUMPABLE) failed");
	}

	/*
	 *init inotify instance, and install inotify watch
	 */
	if ( !init_inotify_instance()) {
