...
struct pxcounters {
	...
	union {
		struct {
			...
			long long rsp_4xx[10]; /*4xx detail errors*/
			long long rsp_5xx[10];/*5xx detail errors*/
		} http;
	} fe, be;				/* FE and BE stats */

	union {
		struct {
			long long l7_delay[4]; /* use l7 switch rule time */					
		}req_delay;
		
		struct {
			long long l4_conn_delay[3];/* time elapsed connect to backend server */	
			long long res_delay[4];/* time delay from recived full client request header and server response header*/
			long long res_data_delay[5];/* time delay from recived full http request header, and finish forward response data*/
		}res_delay;
	} delay;
...
struct srvcounters {
	...
	union {
		struct {
			long long rsp[6];	/* http response codes */
			long long rsp_4xx[10];/*log 4xx detail info*/
			long long rsp_5xx[10];/*log 5xx detail info*/
		} http;
	} p;
