...
struct session {
	...
	struct {
		...
		struct timeval tv_connect;	/* date of connect to backend server*/
		//struct timeval tv_request_header;	/* date the request header arrives, {0,0} if never valid request occurs*/
		struct timeval tv_forward_req;	/* date forward the request to backend server */
		long   rdtsc_request_header;
	

		long  t_queue;			/* delay before the session gets out of the connect queue, -1 if never occurs */
		long  t_connect;		/* delay before the connect() to the server succeeds, -1 if never occurs */

		long  t_l4_delay;		/* delay time before connected to backend server.*/
		long  t_l7_delay;		/* delay time before forward client request to server.*/
		long  t_srv_delay;		/* delay before the first data byte from the server, not include l7 delay*/
		long  t_data_fin;		/* delay time of all the data recvied*/
