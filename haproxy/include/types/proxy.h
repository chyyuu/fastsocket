...
/* values of  7layer rule (acl) load mode */
#define ACL_INIT       0 
/* reload the acl file */
#define ACL_RELOAD     1
...
#define PR_O3_SERVER_FROM_FILE 0x00000001
#define PR_O3_HASH_FILENAME    0x00000002
#define PR_O3_Change2Forward   0x00000004
#define PR_O3_ACL_FROM_FILE    0x00000008
/* end of proxy->option3 */
...
/*
 * bits for reload server file
 */
#define BACKEND_SERVER_INOTIFY 0x00000001
#define ACL_INOTIFY 		   0x00000002 
/*
 * bits for configure check
 */
#define PROXY_CONFIG_VALID 0x0000010
#define PROXY_CONFIG_INVALID 0x0000020
...
struct proxy {
	...
	struct list acl_new;                    /* ACL list for hot load */
	struct list switching_rules_new;        /* content switching rules (chained) for reload*/
	struct list block_cond_new;                 /* early blocking conditions (chained) */
	struct list redirect_rules_new;        /* content switching rules (chained) for reload*/

	struct hdr_exp *req_exp_new;		/* regular expressions for request headers for reload */
	struct hdr_exp *rsp_exp_new;		/* regular expressions for response headers  for reload*/

	struct list req_add_new, rsp_add_new;    /* headers to be added for reload */
	...
	struct server *srv, *unused_srv, defsrv;/* known servers; unused server list,;default server configuration */
...
#endif
	char *hdrname2xforward;     /*hdr name has to change to X-Forwarded-For*/
	int hdrname2xforward_len ;/*hdr name has to change to x-Forwarded-For's len */
	...
	struct hdr_exp *req_cookie_del; /* regular expression for request cookies*/
	struct hdr_exp *rsp_cookie_del; /* regular expression for response cookies*/
	...
	struct {
		...
		char* backend_server_file;	/*file where server_list store*/
		char* acl_file;				/*file where acl_list store*/
		int inotify_fd;     		/* backend server list file notify fd*/
		int flags;
