...
struct global {
	...
	int rlimit_core;	/* default ulimit-c value : 0=unset */
	int mode;
	int last_checks;
	int spread_checks;
	char *chroot;
	...
	int loglev1, loglev2;
	int minlvl1, minlvl2;
	int inotify_fd; /* inotify instance*/
	...
	struct listener* p_stats_sock; /* unix socket listener for statistics,each listener has it's own sock */
	struct proxy *stats_fe;     /* the frontend holding the stats settings */
};

#define ACL_MAX_SEQUENCE_LEN 256
extern char acl_sequence[ACL_MAX_SEQUENCE_LEN];    /* record the acl(7layer) configuration's sequence */
...
extern char lb_name[((MAX_HOSTNAME_LEN + 2)/3) * 4 + 1];
